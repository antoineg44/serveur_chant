<?php
	...
	class FileExplorerFSHelper
	{
		...
		// Refresh folder.
		protected static function ProcessSearchAction(&$options)
		{
			$path = @realpath(rtrim($options["base_dir"], "/"));
			$search_word = self::GetRequestVar("searchword");

			if (!isset($options["search"]) || !$options["search"])  $result = array("success" => false, "error" => self::FETranslate("Operation denied."), "errorcode" => "search_not_allowed");
			else if ($path === false)  $result = array("success" => false, "error" => self::FETranslate("Invalid path specified."), "errorcode" => "invalid_path");
			else
			{
		...
		public static function HandleActions($requestvar, $requestprefix, $basedir, $options)
		{
			...
			if ($action === $requestprefix . "refresh")  $result = self::ProcessRefreshAction($options);		// Celui-là est sélectionné !
			else if ($action === $requestprefix . "search")  $result = self::ProcessSearchAction($options);
