...
(function() {
	...
	var TextareaOverlay = function(parentelem, options) {
		...
		$this.Done = function(etype) {
			...
			{
				if(confirm("Voulez-vous vraiment renommer ?")) {
	...
	window.FileExplorer = function(parentelem, options) {
		...
		var elems = {
            ...
            // Search
			searchwrap: CreateNode('div', ['fe_fileexplorer_search_wrap']),
            searchicon: CreateNode('div', ['fe_fileexplorer_search_icon']),
			searchsegment: CreateNode('div', ['fe_fileexplorer_search_segment']),
			searchsegmentInput: CreateNode('input', [], { id: 'searchtext',type: 'text', minlength: '3', maxlength: '30', placeholder: 'Rechercher' }),
			navtool_cancel: CreateNode('button', ['fe_fileexplorer_navtool_cancel', 'fe_fileexplorer_disabled'], { title: "Annuler", tabIndex: -1 }),
			search_loader: CreateNode('span', ['fe_fileexplorer_loader', 'fe_fileexplorer_invisible']),
		...
		// search
		elems.searchwrap.appendChild(elems.searchicon);
		elems.searchsegment.appendChild(elems.searchsegmentInput);
		elems.searchwrap.appendChild(elems.searchsegment);
		elems.searchwrap.appendChild(elems.search_loader);
		elems.searchwrap.appendChild(elems.navtool_cancel);

		elems.toolbar.appendChild(elems.navtools);
		elems.toolbar.appendChild(elems.pathwrap);
		elems.toolbar.appendChild(elems.searchwrap);
		...
		if ($this.settings.onsearch)  $this.addEventListener('search_folder', $this.settings.onsearch);
		...
		$this.OpenSelectedItems = function() {
			...
			for (var x = 0; x < selnums.length; x++)
			{
				...
				{
					var path = currfolder.GetPathIDs();

					if(entry.id[1] == "/")
					{
						var searchpath = entry.id.split("/");
						entry.id = searchpath[searchpath.length-1];

						var pathnew = [
							""
							];
						for(var i=1; i<searchpath.length-1; i++)
						{
							pathnew.push(searchpath[i]);
						}
						path = pathnew;
					}


					DispatchEvent('open_file', [currfolder, entry, path]);
				}
			}
		...
		$this.SearchFolders = function(forcecurrfolder, searchtext) {
			if (forcecurrfolder)  currfolder.lastrefresh = 0;
			...
			for (var x = historyentry.folderkeys.length; x; x--)
			{
				if (foldermap[historyentry.folderkeys[x - 1]].lastrefresh < ts2)
				{
					if(folderBeforeSearch == null)folderBeforeSearch = foldermap[historyentry.folderkeys[x - 1]];
					DispatchEvent('search_folder', [foldermap[historyentry.folderkeys[x - 1]], !foldermap[historyentry.folderkeys[x - 1]].lastrefresh, searchtext]);
		...
		$this.SetPath = function(newpath) {
			if(newpath[newpath.length-1][0][0] == ".")
			{
				var pathnew = [
					["", "/"]
				  ];
				var searchpath = newpath[newpath.length-1][0].split("/");
				for(var i=1; i<searchpath.length; i++)
				{
					pathnew.push([searchpath[i],searchpath[i]]);
				}
				newpath = pathnew;
			}
			if(newpath)
...
*/
		...
		var resultsearch = function(e) {
			var entries = currfolder.GetEntries();

			SyncCurrFolderToDOM();
			elems.search_loader.classList.add('fe_fileexplorer_invisible');
		}
		...
		var updateSearch = function(e) {
			if(e.target.value.length > 2 && (e.keyCode == 13 || e.keyCode == 32)) {
				...
				currfolder.removeEventListener('set_entries', SetFolderEntriesHandler);
				currfolder.addEventListener('set_entries', resultsearch);

				$this.SearchFolders(true, e.target.value);

				console.log("add");
				elems.navtool_cancel.classList.remove('fe_fileexplorer_disabled');
				elems.search_loader.classList.remove('fe_fileexplorer_invisible');
				elems.navtool_cancel.tabIndex = 0;
			} else if(e.target.value.length == 0) {
				console.log("remove");
				if(folderBeforeSearch != null)
					DispatchEvent('refresh_folder', [folderBeforeSearch, true]);
				else console.log("folderBeforeSearch null");
				elems.navtool_cancel.classList.add('fe_fileexplorer_disabled');
				elems.search_loader.classList.add('fe_fileexplorer_invisible');
			}
		};

		var ClickToCancelSearchHandler = function(e) {
			if (!e.isTrusted)  return;
			...
			elems.navtool_cancel.classList.add('fe_fileexplorer_disabled');
			elems.searchsegmentInput.value = "";
			$this.Focus(true);
		};

		elems.navtool_cancel.addEventListener('mouseup', ClickToCancelSearchHandler);

		elems.searchsegmentInput.addEventListener("keypress", updateSearch);
	...
	var FileExplorerTool_Upload = function(fe) {
		...
		var CreateInputField = function() {
			...
			inputfield = CreateNode('input', ['fe_fileexplorer_hidden'], { type: 'file', multiple: false });		// FIXME: set multiple to false
	...
	var FileExplorerTool_Download = function(fe) {
		...
		return; 		// FIXME : not the good way to disable download
	...
	var FileExplorerTool_ItemCheckboxes = function(fe) {
		...
		fe.GetElements().innerwrap.classList.toggle('fe_fileexplorer_show_item_checkboxes');// FIXME : not the good way, to disable this function at startup on smartphone
	};
