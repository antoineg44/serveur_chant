...
class SSP {
	 ...
	 */
	static function getOrderColum($request, $columns)
	{
		$order = '';
	...
	static function simple ( $request, $conn, $table, $primaryKey, $columns )
	{
		...
		$data = self::sql_exec( $db, $where, $columns,
			 self::getOrderColum($request, $columns), $order, $limit, $request);
		//print_r($data);
		...
		$recordsFiltered = $GLOBALS['nb_item']; //$resFilterLength[0][0];
		...
		$recordsTotal = $GLOBALS['nb_total'];// $resTotalLength[0][0];
	 ...
	 */
	static function sql_exec ( $db, $bindings, $columns, $where, $order, $limit, $request)
	{
		// Argument shifting
		/*if ( $sql === null ) {
			$sql = $bindings;
		...
		$path = "/home/vol2_6/infinityfree.com/if0_36730940/htdocs/pdf/programmes";
		...
		foreach ($iterator as $item) 
		{
			...
			if ($item->isFile()) 
			{
				...
				if ($entry !== false)
				{
					$description = explode("_", $entry["name"]);
					$paroisse = explode("/", $entry["id"]);
					if(sizeof($description) == 3 && sizeof($paroisse) == 3 && $paroisse[1] != "Templates")
					{
						$occasion = explode(".", $description[2]);
						$infos = array(
							"Date" => $description[0],
							"Lieu" => $description[1],
							"Occasion" => $occasion[0],
						...
						if (isset($request['search']) && $request['search']['value'] != '') 
						{
							...
							for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
								$column = $columns[$j];
								if(preg_match('/'.$request['search']['value'].'/i', $infos[$column["db"]]))
									$find = true;
							}
							if($find)
							{
								//if(isset($request['start']) && $i > intval($request['start']) && $i < intval($request['start']) + intval($request['length']) || !isset($request['start']) && $i < intval($request['length']))
									$result[] = $infos;
								$i = $i+1;
							}
						}
		...
		$GLOBALS['order'] = $where;
		$GLOBALS['nb_item'] = count($result);
		$GLOBALS['nb_total'] = $j;

		if($GLOBALS['order'] != "")		// éviter problème !
		{
			// Extract quantities into a separate array
			$quantities = array_map(function ($infos) {
				return $infos[$GLOBALS['order']];
			}, $result);

			// Sort the original array based on the quantities
			if($order == "ASC")
				array_multisort($quantities, SORT_ASC, $result);
			else
				array_multisort($quantities, SORT_DESC, $result);
		}
