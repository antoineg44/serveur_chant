...
async function loadChantsForTemps(tempsLiturgiqueId) {
    const query = new URLSearchParams({ action: 'search_advanced', temps_liturgique_id: String(tempsLiturgiqueId) }).toString();
...
async function fill_list() {
    ...
    try {
        const tempsResponse = await fetch(`${DATA_API}?action=temps_liturgiques`, { credentials: 'include' });
        if (tempsResponse.status === 401) {
            redirectToLogin();
            return;
        }

        const tempsPayload = await tempsResponse.json().catch(() => null);
        if (!tempsResponse.ok || !tempsPayload || !tempsPayload.success) {
            throw new Error((tempsPayload && tempsPayload.message) || `HTTP ${tempsResponse.status}`);
        }

        const tempsLiturgiques = tempsPayload.tempsLiturgiques || [];
        const chantsByTemps = await Promise.all(tempsLiturgiques.map((temps) => loadChantsForTemps(temps.id)));

        tempsLiturgiques.forEach((temps, index) => {
            insert_temps(temps.nom, chantsByTemps[index]);
        });

        activate();
    } catch (error) {
        console.error(error);
        document.getElementById('liste_temps_liturgiques').innerHTML =
            `<li class="p-4 text-red-600">Impossible de charger les chants : ${error.message}</li>`;
    }
}

function insert_temps(nom, chants) {
    ...
    const html_chants = chants.length
        ...
        : '<p class="text-gray-500 py-2">Aucun chant pour ce temps liturgique.</p>';

    const end = "</div></li>";
    document.getElementById('liste_temps_liturgiques').insertAdjacentHTML('beforeend', entete + html_chants + end);
}
